...
**0.2.x architecture note:** positional source editing now delegates to the independent `sumIDE` xBase profile. The interpreter, SQLite/database runtime, console, `--run`, `--check`, and readable `--compile` path remain in sumX.
...
pip install ./sumide-0.2.2.tar.gz
pip install ./sumdiff-0.2.0.tar.gz
pip install ./sumx-0.2.2.tar.gz
```
...
The editor is the shared `sumIDE 0.2.2` shell, with xBase execution supplied by sumX. Its top-level menus are:
...
In both the common sumIDE xBase workspace and the classic sumTUI command environment, `HELP`/F1 open the sumX language help explorer (`F11` maximizes/restores). In sumIDE, Ctrl+F1 is reserved for generic editor help. The command window itself supports `PageUp` and `PageDown` for sumX output scrollback. `Shift+PageUp` / `Shift+PageDown` are aliases when the terminal forwards them; many Linux terminal emulators consume those shortcuts themselves.
